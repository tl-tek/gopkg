// copied and modified from https://github.com/samber/slog-formatter/blob/4ce6c72294ee982ed459d49a2faac326a257181e/formatter_time.go
...
func UnixTimestampFormatter(precision time.Duration) Formatter {
	if !slices.Contains([]time.Duration{time.Nanosecond, time.Microsecond, time.Millisecond, time.Second}, precision) {
		panic("logger formatter: unexpected precision")
	}
